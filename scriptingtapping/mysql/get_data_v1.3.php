...
#File Config
$dbconf = fopen("/var/log/config.json", "r+") or die("Unable to open file!");
...
if(!$db) {
  ...
  file_put_contents('/var/log/get_data.log', $log_1, FILE_APPEND);
  file_put_contents('/var/log/get_data.log', $log_2, FILE_APPEND);
  file_put_contents('/var/log/get_data.log', $log_3, FILE_APPEND);
}
...
if ($conn->connect_error) {
  ...
  file_put_contents('/var/log/get_data.log', $log0, FILE_APPEND);
  file_put_contents('/var/log/get_data.log', $log01, FILE_APPEND);
...
if($conn){
    ...
    $log_5  = date('Y-m-d H:i:s').' - '."(INFO !) - Pengecekan data transaksi yang gagal diambil ...".PHP_EOL;
	  file_put_contents('/var/log/get_data.log', $log_4, FILE_APPEND);
	  file_put_contents('/var/log/get_data.log', $log_5, FILE_APPEND);
}
...
file_put_contents('/var/log/get_data.log', $log_6, FILE_APPEND);

$query = mysqli_query($conn, "SELECT * FROM $tables WHERE initanggal LIKE '%$dates%' AND tax1Amount !='0' ");
$strings = '';
...
if(!empty($strings)){
  ...
  // file_put_contents('/var/log/get_data.log', $log_6, FILE_APPEND);
  file_put_contents('/var/log/get_data.log', $log_7, FILE_APPEND);
  file_put_contents('/var/log/get_data.log', $log_8, FILE_APPEND);
}else{
  ...
  file_put_contents('/var/log/get_data.log', $logx1, FILE_APPEND);
}
...
while($row_ = $rslt->fetchArray()){
  if($row_['keterangan'] == 'berhasil'){
    ...
    file_put_contents('/var/log/get_data.log', $log_11, FILE_APPEND);
    file_put_contents('/var/log/get_data.log', $log_s1, FILE_APPEND);
    file_put_contents('/var/log/get_data.log', $log_s2, FILE_APPEND);
    exit();
  }
}

#Perform queries
$query = mysqli_query($conn, "SELECT * FROM $tables WHERE initanggal like '%$date%' AND tax1Amount !='0' ");
$string = '';
...
file_put_contents('/var/log/get_data.log', $logs1, FILE_APPEND);
file_put_contents('/var/log/get_data.log', $logs2, FILE_APPEND);
}
...
file_put_contents('/var/log/get_data.log', $log11, FILE_APPEND);
file_put_contents('/var/log/get_data.log', $log22, FILE_APPEND);
file_put_contents('/var/log/get_data.log', $log33, FILE_APPEND);
file_put_contents('/var/log/get_data.log', $log44, FILE_APPEND);
}
